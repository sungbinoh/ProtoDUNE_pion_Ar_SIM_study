...
def CheckJobStatus(logfiledir, cycle, jobnumber, hostname):
  ...
  line_JobStart = ""
  for l in log_o:
    # [AnalyzerCore::Loop] Event Loop Started 2018-05-17 19:51:10
    if "Event Loop Started" in l:
      line_JobStart = l.replace("[AnalyzerCore::Loop] Event Loop Started ","")
      break
  ForTimeEst = LASTLINE
  ...
  if FinishString in LASTLINE:
    ...
    for i in range(0,len(log_o)):
      ...
      if "[AnalyzerCore::Loop RUNNING]" in l:
        ForTimeEst = l
        break
    # [AnalyzerCore::Loop] JOB FINISHED 2018-12-06 04:10:37
    line_JobFinished = LASTLINE.replace("[AnalyzerCore::~AnalyzerCore] JOB FINISHED ","")
    EventDone = GetEventDone(ForTimeEst)
    return "FINISHED"+"\tEVDONE:"+EventDone+"\t"+line_JobStart+"\t"+line_JobFinished

  ## 4) [AnalyzerCore::Loop] Event Loop Started 2018-06-04 18:37:57
  elif "Event Loop Started" in LASTLINE:
    return "RUNNING\t"+str(0)+"\tEVDONE:"+str(0)+"\t"+line_JobStart
  
  ## 3) Running
  elif "[AnalyzerCore::Loop RUNNING]" in LASTLINE:
    # [AnalyzerCore::Loop RUNNING] 2011000/38777460 (5.186 %)
    ...
    for it_l in range(0,len(log_o)):
      ...
      if ("[AnalyzerCore::Loop RUNNING]" in l) and ("@" in l):
